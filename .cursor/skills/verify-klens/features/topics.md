...
- **Proof.** Write `landing.png`, `landing.aria.yml`, `topics.json`, `internal.png`, and `open.png` under `artifacts/<run-id>/topics/`. The screenshot shows the `klens` wordmark and `klens-verify-topics`. `GET /clusters/local/topics` lists that topic name.

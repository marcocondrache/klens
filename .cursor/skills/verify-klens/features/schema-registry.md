...
- **Proof.** Screenshot the catalog with the heading and at least one column header (`Subject`, `Latest version`, `Compatibility`). Save `POST /graphql` `schemaSubjects(cluster: "local") { subject latestVersion compatibility }`. The sheet body comes from `subjectSchema(cluster: "local", subject: "<name>") { schema }`.

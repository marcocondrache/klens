...
- **Proof.** Screenshot the catalog with the heading and at least one column header (`Subject`, `Type`, `Compatibility`). Save `POST /graphql` `subjectRows(cluster: "local") { rows { subject id type } sourceHealth { lastError } }`.

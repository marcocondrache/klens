...
- **Open subject.** Click a subject row. A sheet titled with that subject appears. The Schema block shows the schema text and fills the sheet. JSON schemas offer Pretty and Raw. The block has Copy schema, Download schema, and Expand schema. Expand widens the sheet. Version buttons under the schema switch the text and stay visible. There is no `/schemas/<subject>` route.

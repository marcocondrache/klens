...
- **Proof.** Screenshot the catalog with the controller badge. Save `POST /graphql` `cluster(name: "local") { brokers { id host port controller } }`. The body has `controller: true` on one broker.
...
- `Topology::assemble` in `src/kafka/store/tables.rs` sets `controller: None`. The badge and GraphQL `controller: true` are the intended user-facing behavior. A verify broker with no badge is that product gap, not a recipe miss.

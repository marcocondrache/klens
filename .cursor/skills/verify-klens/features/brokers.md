...
- **Proof.** Screenshot the catalog with the controller badge. Save `POST /graphql` `brokerRows(cluster: "local") { id host port controller }`. The body has `controller: true` on one broker.

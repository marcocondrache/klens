...
1. It builds `web/` into `static/`, then runs `cargo build --locked --features ui`. `mise run web:build` is used when `mise` is on `PATH`. Otherwise it uses `bun` or `npm` in `web/`.

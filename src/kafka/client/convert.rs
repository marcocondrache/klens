...
impl MetadataSnapshot {
    /// Reads the already-refreshed metadata cache shared by every krafka
    ...
    pub(super) fn from_krafka(cache: &ClusterMetadata) -> Self {
        Self {
            ...
            topics: cache
                ...
                .into_iter()
                .map(|topic| {
                    // The broker returns partitions in ID order; krafka's
                    // per-topic map does not preserve that, so restore it
                    // rather than exposing hashmap iteration order.
                    ...
                    TopicMetadata {
                        internal: is_internal_topic(&topic.name),
                        name: topic.name,
                        partitions,
                    }
                })

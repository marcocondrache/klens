...
/// Builds krafka's typed auth config from a cluster's security settings.
///
/// `None` means plaintext with no auth. A `Sasl*` protocol with no `sasl`
/// block configured is a configuration error rather than a silent plaintext
/// fallback: the operator asked for SASL, and there is no other way to
/// authenticate once the broker rejects a plaintext connection.
